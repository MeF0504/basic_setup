...
let s:w_b = 0.114 " <- 1.0
let s:thsd = 0.42
function! meflib#color#isdark(r, g, b, verbose=v:false)
    " 0 <= r, g, b <= 1
    let val = (a:r*s:w_r+a:g*s:w_g+a:b*s:w_b)/(s:w_r+s:w_g+s:w_b)
    let cond = val < s:thsd
    if a:verbose
        echo printf("%.3f < %.2f", val, s:thsd)
    endif
    return cond
endfunction
...
function! meflib#color#chk_isdark() abort
    ...
    for g in range(6)
        for r in range(6)
            for b in range(6)
                let i = 36*r+6*g+b+16
                if meflib#color#isdark(r/5.0, g/5.0, b/5.0)
