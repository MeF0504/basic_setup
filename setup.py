...
for fy in files:
    ...
    if os.path.exists(spath):
        if 'zshrc_file' in spath:
            fcopy(spath,os.path.expanduser(files[fy]),link=True)
...
for fy in files:
    ...
    if os.path.exists(vpath):
        ...
        if 'vimrc_file' in vpath:
            fcopy(vpath,os.path.expanduser(files[fy]),link=True)

        else:
            fcopy(vpath,os.path.join(rcdir,files[fy]),link=True)

        """
        if 'vimrc_color' in vpath:
            fcopy(vpath,os.path.join(rcdir,vpath
